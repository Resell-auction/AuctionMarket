...
public class PaymentController {
    ...
    public ResponseEntity<Void> pay(Long paymentId, @RequestBody PaymentRequest request) {
        paymentService.confirmPayment(paymentId, request);
        return ResponseEntity.ok().build();
    }

    @PostMapping("/v1/payments/{paymentId}/refund")
    public ResponseEntity<Void> refund(Long paymentId, @RequestBody RefundRequest request) {
